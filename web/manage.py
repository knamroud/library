...
def main():
    ...
    if "DJANGO_SETTINGS_MODULE" not in os.environ:
        if len(sys.argv) > 1 and sys.argv[1] == "test":
            os.environ.setdefault("DJANGO_SETTINGS_MODULE", "library.settings.test")
        elif len(sys.argv) > 1 and sys.argv[1] == "runserver":
            os.environ.setdefault("DJANGO_SETTINGS_MODULE", "library.settings.development")
        else:
            os.environ.setdefault("DJANGO_SETTINGS_MODULE", "library.settings.production")
    try:
